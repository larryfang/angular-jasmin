...
StaticServlet.prototype.handleRequest = function(req, res) {
    ...
    if (!IGNORED.test(path) && (match = path.match(REWRITE))) {
        path = path.replace(match[0], '/test.html');
    ...
    fs.stat(path, function(err, stat) {
        ...
        if (stat.isDirectory())
            return fs.stat(path + 'test.html', function(err, stat) {
                // send test.html if exists
                if (!err)
                    return self.sendFile_(req, res, path + 'test.html');
